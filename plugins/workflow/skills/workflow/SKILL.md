...
> **確定的な下支え（フック）**: 上記はあくまで運営モードでの能動的な記録。これとは別に `Stop` フック（`hooks/notes-record.js`）が、応答終了ごとに「その日の利用者依頼」を `notes/YYYY-MM-DD.md` に **1 セッション 1 行で upsert** して最低限の履歴を保証する（自動記録が漏れても反復検知が機能するように）。フックは LLM が書いた他行は触らない。保持期間（既定 14 日・`config.yaml` の `notes_retention_days`）を過ぎた**日次ログのみ**自動削除する（トピックノートは残す）。
