...
- **数値はすべてトークン数であり、金額ではない**。「API換算（旧称: 課金相当）」を金額・請求額として案内しない。Pro/Max プランは月額定額制のため、消費が増えても**追加請求は発生しない**（利用制限に当たりやすくなるだけ）。利用者が「サブスクを超えて請求される」と心配したら、この定額制の説明を必ず添える（実際に「課金相当 392.5万」を金額と誤読された事例あり）。
- 「制限相当」は cache 読取も満額で数える推計、「API換算」は cache 読取 0.1 掛け。**両者は別物**なので混ぜて語らない。
