...
- アバターは**動く**：常時ふわふわ揺れ（＋SVG フォールバック時はまばたき）、ページを開くとあいさつ吹き出し、クリックで「話す（レベル・連続稼働・次バッジに応じたセリフ）／ドキドキ（💗）／ワクワク（✨ジャンプ）／1回転」のいずれかで反応する。すべて生成 HTML 同梱の CSS/JS（外部依存ゼロ）で、OS の「視差効果を減らす」設定（prefers-reduced-motion）では装飾アニメーションを自動停止する。
...
- 出力 JSON は `out`（絶対パス）・`url`（`file://` リンク）・`markdown`（そのまま貼れる完成リンク `[🎮 ダッシュボードを開く](file://…)`）を含む。**OS の open コマンドは打たない**。ダッシュボードの案内方法は Step 3 の必須ルールに従う。
...
**ダッシュボードへの案内は必ずクリック可能なリンクで締める（最重要・違反禁止）**：

- 最後に、`avatar-render.js` 出力の **`markdown` の値をそのまま 1 行で出力する**（例: `[🎮 ダッシュボードを開く](file:///Users/you/.belta/dashboard.html)`）。
- **生のファイルパス（`/Users/.../dashboard.html` や `~/.belta/dashboard.html`）を本文に書いてはいけない**（クリックできず、利用者が毎回コピペする羽目になる。これがまさに避けたい挙動）。
- 「ダッシュボードはこちら：」のあとにパスを貼る、という出し方も禁止。必ず Markdown リンク（`markdown` の値）で出す。
- `markdown` が空（`url` 取得不可）のときに限り、`out` のパスを文言で案内する（fail-open）。
