...
- Belta の社員（情報システム部を中心に、Phase -1 では情シス 2〜3 名でドッグフード中）。
- Claude Code を業務で使う方。エンジニアでなくても、`gh auth login --web` の 1 コマンドと claude.ai のブラウザ認可ができれば利用できます。

::: tip Phase -1（社内ドッグフード）
本プラグインは現在 Phase -1（社内ドッグフード）です。情シス数名で動作確認し、経営承認会議向けの実測データを取得する段階です。機能・ドキュメントは随時更新されます。
:::
